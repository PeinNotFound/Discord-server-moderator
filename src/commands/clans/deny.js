...
module.exports = {
    name: 'deny',
    description: '[Leader/Co-Leader] Remove permissions from users/roles',
    usage: '&deny <user/role/leader> <@user/@role/id>',
    permission: 'member',
    ...
    async execute(message, args, client) {
        ...
        if (!['user', 'role', 'leader'].includes(type)) {
            return message.reply('❌ Usage: `&deny <user/role/leader> <@user/@role/id>`');
        }
        ...
        try {
            ...
            if (type === 'user') {
                ...
                return message.reply(`✅ Removed **${targetMember.user.tag}**'s access to the clan chat!`);
            }
            ...
            if (type === 'role') {
                ...
                return message.reply(`✅ Removed **${targetRole.name}** role's access to the clan chat!`);
            }
            ...
            if (type === 'leader') {
                ...
                return message.reply(`✅ Removed **${targetMember.user.tag}**'s chat management permissions!`);
            }
        } catch (error) {
