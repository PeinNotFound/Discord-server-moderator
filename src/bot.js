...
// Store timeout IDs for auto-actions so they can be cleared when manually undone
const autoUnmuteTimeouts = new Map(); // userId -> timeoutId
const autoUnjailTimeouts = new Map(); // userId -> timeoutId
...
client.once('ready', () => {
    ...
    verification.initVerification(client, botConfig);
});
...
client.on('messageDelete', (message) => {
    ...
    deletedMessages.set(channelId, snipeData);
...
client.on('guildMemberUpdate', async (oldMember, newMember) => {
    ...
    if (oldMember.nickname !== newMember.nickname) {
        ...
        // Guard against missing member or guild
...
client.on('voiceStateUpdate', async (oldState, newState) => {
    ...
    // Guard against deleted guilds or channels
    ...
    // Handle verification room joins
    try {
        await verification.handleVerificationRoomJoin(client, newState);
    } catch (error) {
        console.error('Failed to handle verification room join:', error);
    }
    ...
    if (oldState.channelId && newState.channelId && oldState.channelId !== newState.channelId) {
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                { name: '👤 User', value: `${member.user.tag}`, inline: true },
                { name: '🆔 User ID', value: `${member.id}`, inline: true },
                { name: '📤 From Channel', value: oldState.channel?.name || 'Unknown Channel', inline: false },
                { name: '📥 To Channel', value: newState.channel?.name || 'Unknown Channel', inline: false },
                { name: '👨‍💼 Performed By', value: moderator, inline: true },
                { name: '⏰ Timestamp', value: `<t:${Math.floor(Date.now() / 1000)}:F>`, inline: true }
            )
            .setThumbnail(member.user.displayAvatarURL())
...
client.on('messageCreate', async (message) => {
    ...
    try {
        ...
        if (command === 'ban') {
            ...
            if (!target && userId) {
                ...
                try {
                    ...
                    if (!reason) return await safeReply(message, '❌ Please provide a reason for the ban! Usage: `!ban @user|user_id [reason]`');
                    ...
                    try {
                        ...
                        return await safeReply(message, { embeds: [successEmbed] });
                    } catch (banError) {
                        return await safeReply(message, `❌ Failed to ban user: ${banError.message}`);
                    }
                }
            }
            ...
            if (!target) return await safeReply(message, '❌ Please mention a user to kick!');
            
            const reason = args.slice(1).join(' ');
            if (!reason) return await safeReply(message, '❌ Please provide a reason for the kick! Usage: `!kick @user [reason]`');
            ...
            try {
                ...
                if (error.code === 50013) {
                    await safeReply(message, '❌ I don\'t have permission to kick users! Please give me the **Kick Members** permission.');
                } else {
            ...
            const userId = args[0];
            if (!userId) return await safeReply(message, '❌ Please provide a user ID to unban! Usage: `!unban [user_id] [reason]`');
            ...
            try {
                ...
                if (!bannedUser) {
                    return await safeReply(message, '❌ User is not banned or user ID is invalid!');
                }
                ...
                if (error.code === 50013) {
                    await safeReply(message, '❌ I don\'t have permission to unban users! Please give me the **Ban Members** permission.');
                } else {
            ...
            try {
                ...
                const unjailTimeoutId = setTimeout(async () => {
                    try {
                        // Check if timeout was cleared (manually unjailed)
                        if (!autoUnjailTimeouts.has(target.id)) {
                            return;
                        }
                        ...
                        // Check if user still has the jail role (might have been manually unjailed)
                        if (!member.roles.cache.has(jailRole.id)) {
                            // User was already unjailed, don't send DM
                            jailedUsers.delete(target.id);
                            delete persistentData.jailedUsers[target.id];
                            saveData(persistentData);
                            autoUnjailTimeouts.delete(target.id);
                        ...
                        // Clear timeout from map after execution
                        autoUnjailTimeouts.delete(target.id);
                    } catch (error) {
                ...
                // Store timeout ID so it can be cleared if manually unjailed
                autoUnjailTimeouts.set(target.id, unjailTimeoutId);
                
            } catch (error) {
                if (error.code === 50013) {
                    await safeReply(message, '❌ I don\'t have permission to manage roles! Please give me the **Manage Roles** permission.');
                } else {
        ...
        // Verification Commands
            ...
            // Verify user (male/neutral)
            const result = await verification.verifyUser(message, null, 'verified');
            // If result is undefined (no permission), treat as unknown command
            if (result === undefined) {
                return await safeReply(message, 'Unknown command. Use `!help` for a list of commands.');
            }
        }
        else if (command === 'vg') {
            // Verify user (female)
            const result = await verification.verifyUser(message, null, 'female');
            // If result is undefined (no permission), treat as unknown command
            if (result === undefined) {
                return await safeReply(message, 'Unknown command. Use `!help` for a list of commands.');
            }
        }
        else if (command === 'vhelp') {
            // Show verification help
            const result = await verification.showVerificationHelp(message);
            // If result is undefined (no permission), treat as unknown command
            if (result === undefined) {
                return await safeReply(message, 'Unknown command. Use `!help` for a list of commands.');
            }
                ...
                if (warningCount >= 3) {
                    try {
                        ...
                        if (jailRole) {
                            ...
                            const autoUnjailTimeoutId = setTimeout(async () => {
                                try {
                                    // Check if timeout was cleared (manually unjailed)
                                    if (!autoUnjailTimeouts.has(target.id)) {
                                        return;
                                    ...
                                    // Get jail role again (might have changed)
                                    let currentJailRole;
                                    if (botConfig.jailRoleId && botConfig.jailRoleId !== 'your_jail_role_id_here') {
                                        currentJailRole = message.guild.roles.cache.get(botConfig.jailRoleId);
                                    } else {
                                        currentJailRole = message.guild.roles.cache.find(role => role.name === 'Jailed');
                                    }
                                    
                                    // Check if user still has jail role (might have been manually unjailed)
                                    if (!currentJailRole || !member.roles.cache.has(currentJailRole.id)) {
                                        // User was already unjailed, don't send DM
                                        jailedUsers.delete(target.id);
                                        delete persistentData.jailedUsers[target.id];
                                        saveData(persistentData);
                                    ...
                                    if (storedJailData && storedJailData.originalRoles.length > 0) {
                                        ...
                                        await member.roles.remove(currentJailRole);
                                    }
                            ...
                            // Store timeout ID so it can be cleared if manually unjailed
                            autoUnjailTimeouts.set(target.id, autoUnjailTimeoutId);
                            ...
                            await safeReply(message, { embeds: [autoJailSuccessEmbed] });
                        } else {
        ...
        // !unwarn command - Remove a warning from a user
            ...
            if (!target) return await safeReply(message, '❌ Please mention a user or provide a user ID to unwarn!');
            ...
            try {
                ...
                if (warningIndex === -1) {
                    ...
                    return await safeReply(message, `❌ Invalid warning number! ${target.user.tag} has ${userWarnings.length} warning(s). Use \`!unwarn @user [1-${userWarnings.length}]\``);
                }
                ...
                await safeReply(message, `❌ Failed to remove warning: ${error.message}`);
            }
        }
        ...
        // Mute Command - Mutes user in both text and voice with time limit
            ...
            if (isNaN(muteTime) || muteTime < 1 || muteTime > 30) {
                return await safeReply(message, '❌ Mute time must be between 1 and 30 minutes! Usage: `!mute @user [1-30]`');
            }
            ...
            try {
                ...
                const unmuteTimeoutId = setTimeout(async () => {
                    try {
                        // Check if timeout was cleared (manually unmuted)
                        if (!autoUnmuteTimeouts.has(target.id)) {
                            return;
                        }
                        ...
                        // Check if user still has muted role (might have been manually unmuted)
                        const mutedRole = message.guild.roles.cache.get(botConfig.mutedRoleId);
                        if (!mutedRole || !member.roles.cache.has(mutedRole.id)) {
                            // User was already unmuted, don't send DM
                            autoUnmuteTimeouts.delete(target.id);
                            return;
                        }
                ...
                // Store timeout ID so it can be cleared if manually unmuted
                autoUnmuteTimeouts.set(target.id, unmuteTimeoutId);
            } catch (error) {
                if (error.code === 50013) {
                    await safeReply(message, '❌ I don\'t have permission to mute users! Please give me the **Moderate Members** and **Manage Roles** permissions.');
                } else {
        ...
        // Unmute Command - Unmutes user from both text and voice
            ...
            try {
                // Clear auto-unmute timeout if it exists
                if (autoUnmuteTimeouts.has(target.id)) {
                    clearTimeout(autoUnmuteTimeouts.get(target.id));
                    autoUnmuteTimeouts.delete(target.id);
                }
                ...
                await safeReply(message, '❌ Failed to unmute user. Check my permissions!');
            }
        }
            ...
            try {
                ...
                await safeReply(message, `🔇 Force muted ${mutedCount} members in your voice channel!`);
            } catch (error) {
            ...
            try {
                ...
                await safeReply(message, `🔊 Force unmuted ${unmutedCount} members in your voice channel!`);
            } catch (error) {
            ...
            if (!channelId) return await safeReply(message, '❌ Please provide a channel ID!');
        ...
        // !rank command - View rank and progress (ADMIN ONLY - can view others)
            ...
            if (!hasRankAdminRole(message.member)) {
                return await safeReply(message, '❌ You don\'t have permission to use this command! Only rank admins can view ranks.\n**Tip:** Use `!points` to view your own rank and points.');
            }
            ...
            if (!target) {
                return await safeReply(message, '❌ Please mention a user or provide a user ID!\nUsage: `!rank @user` or `!rank <user_id>`');
            }
            ...
            try {
                ...
                await safeReply(message, { embeds: [rankEmbed] });
            } catch (error) {
                await safeReply(message, '❌ Failed to fetch rank information!');
            }
        }
        ...
        // !points command - View own points (SELF ONLY)
            ...
            try {
                ...
                await safeReply(message, '❌ Failed to fetch points information!');
            }
        }
        ...
        // !points_add command - Add points (RANK ADMIN ONLY)
            ...
            if (!hasRankAdminRole(message.member)) {
                return await safeReply(message, '❌ You don\'t have permission to use this command! Only rank admins can add points.');
            }
            ...
            // Check if target is a staff member
            if (!isStaffMember(target, botConfig)) {
                return await safeReply(message, `❌ **${target.user.tag}** is not a staff member! Points can only be given to staff members with at least one staff rank role.`);
            }
            ...
            try {
                ...
                await safeReply(message, { embeds: [pointsEmbed] });
                ...
                await safeReply(message, '❌ Failed to add points!');
            }
        }
        ...
        // !points_minus command - Remove points (RANK ADMIN ONLY)
            ...
            if (!hasRankAdminRole(message.member)) {
                return await safeReply(message, '❌ You don\'t have permission to use this command! Only rank admins can remove points.');
            }
            ...
            // Check if target is a staff member
            if (!isStaffMember(target, botConfig)) {
                return await safeReply(message, `❌ **${target.user.tag}** is not a staff member! Points can only be managed for staff members with at least one staff rank role.`);
            }
            ...
            try {
                ...
                await safeReply(message, '❌ Failed to remove points!');
            }
        }
        ...
        // !rank_help command - Show rank system information with pagination
            ...
            // If reply failed (message deleted), skip collector
            if (!reply) return;
        ...
        // Help Command - Paginated
            ...
            // If response failed (message deleted), skip collector
            if (!response) return;
        ...
        // Manual Unjail Command
            ...
            try {
                ...
                // Clear auto-unjail timeout if it exists
                if (autoUnjailTimeouts.has(target.id)) {
                    clearTimeout(autoUnjailTimeouts.get(target.id));
                    autoUnjailTimeouts.delete(target.id);
                }
        ...
        // Unknown command
            ...
            return await safeReply(message, '❓ Unknown command! Type `!help` to see all available commands.');
        }
        
    } catch (error) {
        await safeReply(message, '❌ An error occurred while executing the command!');
    }
});
